...
| 2 | [Generic Marketplace](cases/marketplace/) | E-commerce returns / promotions / logistics | Full Kimball model (10 dims + 6 facts + 3 analytical marts) with a Streamlit dashboard powered by **AI insights from Claude** — see [`app/`](app/). |
...
## Live dashboard (Case 2)

A production-shaped Streamlit dashboard sits on top of the marketplace marts:

- 5 tabs in the OP1/OP2 review style: Overview · Returns · Promotions · Logistics · AI Insights
- Plotly charts (heatmap, scatter, time series, stacked bars)
- **AI narrative** generated by Anthropic Claude — receives only pre-aggregated facts (anti-hallucination by design), cached on disk for 60 minutes per signature
- Graceful degradation: works without an API key, only the AI tab loses its narrative

```bash
cd dbt && dbt build --target duckdb && cd ../app
pip install -r requirements_dashboard.txt
cp .env.example .env       # add ANTHROPIC_API_KEY (optional)
streamlit run dashboard.py
```

See [`app/README.md`](app/README.md) for the architecture write-up.
